...
const Location = ()=>{
    ...
    return (
        <div className="p-3 text-white border border-white/30 rounded-xl flex flex-col justify-between">
            <div 
                key={busData?.routeNo}  
                ...
                <div className="text-xs text-white/70 text-right pt-3">
                    <p>ETA {busData?.ETA} min</p>
                    <Link to={"/tracking"} onClick={()=>settab("tracking")}><p className="underline">View on Map</p></Link>
                </div>
            </div>
